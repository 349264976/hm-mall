...
//        Set<Long> itemIds = vos.stream().map(CartVO::getItemId).collect(Collectors.toSet());
          List<Long> itemIds = vos.stream().map(CartVO::getItemId).distinct().collect(Collectors.toList());
...
//        URI uri = serviceInstance.getUri();
        // 2.查询商品
//        ResponseEntity<List<ItemDTO>> responseEntity = restTemplate.exchange(/*"http://localhost:8081/" +*/
//                uri+ "/items?ids={ids}"
//                , HttpMethod.GET
//                , null
//                , new ParameterizedTypeReference<List<ItemDTO>>() {
//                }
//                ,CollectionUtil.join(itemIds,","));
//
